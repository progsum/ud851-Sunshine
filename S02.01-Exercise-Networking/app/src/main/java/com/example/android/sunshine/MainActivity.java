...
public class MainActivity extends AppCompatActivity {
    ...
    public void getWaetherData(){
        //getPrefLoc
        String loc = SunshinePreferences.getPreferredWeatherLocation(this);
        new OpenWeatherAPIAsyncTask().execute(loc);
    }
    // TODO (5) Create a class that extends AsyncTask to perform network requests
    public class OpenWeatherAPIAsyncTask extends AsyncTask<String, Void, String[]> {
        // TODO (6) Override the doInBackground method to perform your network requests
        @Override
        protected String[] doInBackground(String... strings) {
            URL urlWithLocQuery = NetworkUtils.buildUrl(strings[0]);
            try {
                String response = "{list:["+NetworkUtils.getResponseFromHttpUrl(urlWithLocQuery)+"]}";
        ...
        protected void onPostExecute(String[] strings) {
            for (String string : strings) {
                mWeatherTextView.append(string.concat("\n"));
            }
        }
    }
}